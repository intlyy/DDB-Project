...
vector<TCC> GetTCCListTest(string sql_statement) {
    // metadata //
    map<string,Fragment> FragmentMap = GetETCDFragment();         //得到各个站点的信息（etcd信息）
    vector<string> table_list = GetTableList(sql_statement);          //得到所有表名
...
QTree SELECT(string sql_statement, int treeid) {
    ...
    for (int i = 0; i < TCCList.size(); i++) {
        ...
        if (TCCList[i].fratype == "H") {
            ...
            for (int k = 1; k <= frags.size(); k++) {
                node.children.push_back(iid-k);
            }
            node.sql_statement = "SELECT * FROM tree_" + to_string(treeid) + "_node_" + to_string(node.children[0]);
            for (int k = 1; k < frags.size(); k++) {
                node.sql_statement += " UNION ALL SELECT * FROM tree_" + to_string(treeid) + "_node_" + to_string(node.children[k]);
            }
            ...
            for (int k = 1; k < frags.size(); k++) { 
                ...
                node.children.clear();
                node.children.push_back(iid-1);
                node.children.push_back(iid-2*k);
                iid += 1;
                tree.push_back(node);
            }
    ...
    for (int i = 0; i < join_condition_list.size(); i++) {
        ...
        node.id = iid;
        node.children.clear();
        node.children.push_back(TableMap[table_1]);
        node.children.push_back(TableMap[table_2]);
    ...
    for (int i = 0; i < tree.size(); i++) {
        ...
        for(int j = i+1; j < tree.size(); j++) {
            if (JudgeNotinInt(tree[j].children, node_id) == 0) {
...
/*
QTree SELECTLocal(string sql_statement,int treeid,map<string,Fragment> FragmentMap,map<string,string> TableKeyMap) {
    vector<TCC> TCCList = GetTCCListTest(sql_statement);
    // TraverseTCCList(TCCList);
